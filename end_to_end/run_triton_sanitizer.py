#!/usr/bin/env python3
"""
Run triton-sanitizer end-to-end experiments for all repositories.
Results are saved to end_to_end/results/triton_sanitizer/
"""
...
# Get triton-sanitizer configurations from registry
ENV_CONFIGS = get_configs_by_group("triton_sanitizer")

# Memory profiling prefix
MEMORY_PROFILE_PREFIX = "/usr/bin/time -v"


class TritonSanitizerRunner:
    ...
    def run_single_test(self, test_info, env_config_key):
        ...
        # Always use triton-sanitizer prefix, optionally add memory profiling
    ...
    def run_all_tests(self, repositories, whitelists=None):
        """Run all tests with triton-sanitizer configurations."""
        ...
        print(f"\nRunning tests with {len(ENV_CONFIGS)} triton-sanitizer configurations")
        print("=" * 60)
    ...
    def save_results_csv(self):
        ...
        csv_file = self.script_dir / "results" / "results_triton_sanitizer.csv"
    ...
    def print_summary(self):
        ...
        print("\n" + "=" * 60)
        print("TRITON-SANITIZER TEST SUMMARY")
        print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description="Run triton-sanitizer end-to-end experiments")
    parser.add_argument(
        "--memory",
        action="store_true",
    ...
    print("=" * 60)
    print("Running Triton-Sanitizer End-to-End Experiments")
    if args.memory:
        print("Memory profiling: ENABLED")
    print("=" * 60)
    print()

    runner = TritonSanitizerRunner(enable_memory=args.memory)
    ...
    print("\n" + "=" * 60)
    print("Triton-sanitizer experiments completed!")
