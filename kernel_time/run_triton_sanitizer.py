#!/usr/bin/env python3
"""
Run triton-sanitizer kernel time experiments for all repositories.
Results are saved to kernel_time/results/triton_sanitizer/
"""
...
# Get kernel_time triton-sanitizer configuration from registry
ENV_CONFIGS = OrderedDict([
    (key, config) for key, config in get_configs_by_group("kernel_time").items()
    if "triton-sanitizer" in config["name"]
])


class KernelTimeTritonSanitizerRunner:
    def __init__(self):
    ...
    def check_triton_sanitizer(self):
        """Check if triton-sanitizer is available."""
        if shutil.which("triton-sanitizer") is None:
    ...
    def extract_kernel_time_from_log(self, log_file):
        ...
        Parses all "Triton-Viz: execution time for ...: X.XXX ms" lines and sums them.
        Returns (total_time_ms, kernel_count) tuple.
        """
        pattern = re.compile(r'Triton-Viz: execution time for [^:]+: ([\d.]+) ms')
    ...
    def run_single_test(self, test_info, env_config_key):
        ...
        if repo_name == "tritonbench" and config.get("special_handling"):
            ...
            # Note: triton-sanitizer uses ENABLE_TIMING instead of ENABLE_TRITON_PROFILER
        ...
        # Always use triton-sanitizer prefix
        command_prefix = env_config.get("command_prefix", "triton-sanitizer")
        if command_prefix:
    ...
    def run_all_tests(self, repositories, whitelists=None):
        """Run all tests with triton-sanitizer kernel time configurations."""
        ...
        print(f"\nRunning tests with {len(ENV_CONFIGS)} triton-sanitizer kernel time configurations")
        print("=" * 60)
    ...
    def save_results_csv(self):
        ...
        csv_file = self.output_base_dir / "results_triton_sanitizer.csv"
    ...
    def print_summary(self):
        ...
        print("\n" + "=" * 60)
        print("KERNEL TIME TRITON-SANITIZER TEST SUMMARY")
        print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description="Run triton-sanitizer kernel time experiments")
    parser.add_argument(
        "--repo",
        type=str,
    ...
    print("=" * 60)
    print("Running Triton-Sanitizer Kernel Time Experiments")
    print("=" * 60)
    print()

    runner = KernelTimeTritonSanitizerRunner()
    ...
    print("\n" + "=" * 60)
    print("Triton-sanitizer kernel time experiments completed!")
