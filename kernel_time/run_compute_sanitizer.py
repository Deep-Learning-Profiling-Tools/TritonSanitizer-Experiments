#!/usr/bin/env python3
"""
Run compute-sanitizer kernel time experiments for all repositories.
Results are saved to kernel_time/results/compute_sanitizer/
"""
...
# Get kernel_time compute-sanitizer configuration from registry
ENV_CONFIGS = OrderedDict([
    (key, config) for key, config in get_configs_by_group("kernel_time").items()
    if "compute-sanitizer" in config["name"]
])


class KernelTimeComputeSanitizerRunner:
    def __init__(self):
    ...
    def run_single_test(self, test_info, env_config_key):
        ...
        # Always use compute-sanitizer prefix
        command_prefix = env_config.get("command_prefix", "compute-sanitizer")
        if command_prefix:
    ...
    def run_all_tests(self, repositories, whitelists=None):
        """Run all tests with compute-sanitizer kernel time configurations."""
        ...
        print(f"\nRunning tests with {len(ENV_CONFIGS)} compute-sanitizer kernel time configurations")
        print("=" * 60)
    ...
    def save_results_csv(self):
        ...
        csv_file = self.output_base_dir / "results_compute_sanitizer.csv"
    ...
    def print_summary(self):
        ...
        print("\n" + "=" * 60)
        print("KERNEL TIME COMPUTE-SANITIZER TEST SUMMARY")
        print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description="Run compute-sanitizer kernel time experiments")
    args = parser.parse_args()

    print("=" * 60)
    print("Running Compute-Sanitizer Kernel Time Experiments")
    print("=" * 60)
    print()

    runner = KernelTimeComputeSanitizerRunner()
    ...
    print("\n" + "=" * 60)
    print("Compute-sanitizer kernel time experiments completed!")
